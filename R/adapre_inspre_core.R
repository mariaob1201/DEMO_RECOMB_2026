...
inspre_worker <- function(X, W = NULL, rho = 1.0, lambda = 0.01,
                          ...
                          constraint = "UV", DAG = FALSE, adaptive_lambda = FALSE, beta_obs = NULL) {
  D <- nrow(X)
  if (is.null(W)) {
  ...
  for (iter in 1:its) {
    if (constraint == "UV") {
      U_next <- fit_U_UV_const(WX, W, V, U, theta, rho, 0, solve_its, DAG, ncores, adaptive_lambda, beta_obs)
      V_next <- fit_V_UV_const(V, U_next, theta, rho, lambda, solve_its, DAG, ncores, adaptive_lambda, beta_obs)
      ...
      V_next <- fit_V_VU_const(V, U_next, theta, rho, lambda, solve_its, DAG, ncores, adaptive_lambda, beta_obs)
...
fit_inspre_sequence <- function(X, lambda, W = NULL, rho = 1.0,
                                ...
                                warm_start = FALSE, constraint = "UV", DAG = FALSE, adaptive_lambda = FALSE, beta_obs = NULL) {
  ...
  for (i in 1:length(lambda)) {
    ...
    inspre_res <- inspre_worker(
      ...
      solve_its = solve_its, ncores = ncores, constraint = constraint, DAG = DAG, adaptive_lambda = adaptive_lambda, beta_obs = beta_obs
    )
    if (verbose) {
      cat(sprintf(
...
inspre <- function(X, W = NULL, rho = 10.0, lambda = NULL,
                   ...
                   warm_start = TRUE, min_nz = 0.01, constraint = "UV", DAG = FALSE, adaptive_lambda = FALSE, beta_obs = NULL) {
  D <- ncol(X)
  if (any(is.na(X))) {
    if (is.null(W)) {
      W <- matrix(1L, nrow = D, ncol = D)
  ...
  if (!is.null(alpha)) {
    ...
    if (alpha > 0) {
      ...
      L0 <- fit_inspre_sequence(
        ...
        ncores = ncores, warm_start = warm_start, constraint = constraint, DAG = DAG, adaptive_lambda = adaptive_lambda, beta_obs = beta_obs
      )$L
  ...
  if (cv_folds > 0) {
    ...
    for (i in 1:cv_folds) {
      ...
      cv_res <- fit_inspre_sequence(
        ...
        solve_its = solve_its, ncores = ncores, warm_start = warm_start, constraint = constraint, DAG = DAG, adaptive_lambda = adaptive_lambda, beta_obs = beta_obs
      )
...
fit_inspre_from_R <- function(R_tce, W = NULL, rho = 100.0, lambda = NULL,
                              ...
                              ncores = 1, warm_start = FALSE, min_nz = 0.01, constraint = "UV", DAG = FALSE, beta_obs = NULL, adaptive_lambda = FALSE) {
  D <- dim(R_tce)[1]
  inspre_res <- # inspre::
    inspre(
      ...
      tau = tau, solve_its = solve_its, ncores = ncores, warm_start = warm_start, min_nz = min_nz, constraint = constraint, DAG = DAG, adaptive_lambda = adaptive_lambda, beta_obs = beta_obs
    )
...
fit_inspre_from_X <- function(X, targets, weighted = TRUE, max_med_ratio = NULL, filter = TRUE,
                              ...
                              cv_folds = 0, mu = 10, tau = 1.5, solve_its = 10,
                              ncores = 1, warm_start = FALSE, min_nz = 0.01, constraint = "UV", DAG = FALSE, adaptive_lambda = FALSE) {
  D <- ncol(X)
  ...
  full_res <- fit_inspre_from_R(R_hat,
    ...
    ncores = ncores, warm_start = warm_start, min_nz = min_nz, constraint = constraint, DAG = DAG, adaptive_lambda = adaptive_lambda, beta_obs = beta_obs
  )
  ...
  if (cv_folds > 0) {
    ...
    for (i in seq_along(folds)) {
      ...
      cv_res <- fit_inspre_from_R(R_hat_cv,
        ...
        verbose = verbose, train_prop = 1,
        cv_folds = 0, mu = mu, tau = tau, solve_its = solve_its,
        ncores = ncores, warm_start = warm_start, min_nz = min_nz, constraint = constraint, DAG = DAG, beta_obs = beta_obs, adaptive_lambda = adaptive_lambda
      )
      V_nz <- abs(cv_res$V) > min_nz
      xi_mat <- xi_mat + V_nz
      X_test <- X[fold, ]
